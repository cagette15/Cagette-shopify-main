...
import type { ActivateAccountApiEndpoint } from 'app/api/accounts/activate/route';
...
export default function ActivateAccountPage() {
  async function onSubmit(ev: FormEvent<HTMLFormElement>) {
    ev.preventDefault();
    const res = await submit<ActivateAccountApiEndpoint>(ev.currentTarget);
    ...
    <section className="flex items-stretch text-white md:min-h-[90vh] ">
      ...
      <div className="z-0 flex w-full items-center justify-center bg-black/95 px-0 text-center md:bg-cagette md:px-16 lg:w-1/2">
        ...
        <div className="z-20 w-full py-6 pt-20 md:pt-0">
          ...
          <form
            method="post"
            action="/api/accounts/activate-account"
            onSubmit={onSubmit}
          ...
          >
            <div className="pb-2 pt-4">
              <input
                ...
                id="email"
                placeholder="Email"
                className="block w-full rounded-sm bg-gray-200 p-4 text-lg"
              />
            </div>
            <div className="pb-2 pt-4">
              <input
